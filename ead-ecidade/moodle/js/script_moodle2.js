...
//$( ".block_tree.list").append('<p class="tree_item leaf hasicon"><a href="http://moodle.cloud.citta.org.br/moodle/login/logout.php?"><img class="smallicon navicon" alt="" src="http://moodle.cloud.citta.org.br/moodle/theme/image.php/clean/core/1452782018/i/navigationitem"><span class="item-content-wrap">SAIR</span></a></p>');
...
//.type_system depth_2 item_with_icon current_branch").css( "display", "none" );



// Atualizar interface dos botões de auto inscrição - matricular
cursos = $('.enrolmenticons > .smallicon');
...
for (i=0; i<=itens_admin.length; i++){ 
	try {var link_desmatricular = itens_admin[i].href; 
	if (link_desmatricular.indexOf('unenrolself') > -1){ 
	//	$(".course-content").append('<div class="button-matricular" onclick="window.location=\''+ link_desmatricular +  +'\';">DESMATRICULAR</div>');  console.log(link_desmatricular);
	$(".course-content").append('<div class="button-matricular button2" onclick="window.location=\''+ itens_admin[i].href +'\';">DESMATRICULAR</div>'); console.log(link_desmatricular);
	}
	} catch (err) {}
}
...
if (id_pagina == "page-course-view-topics"){
	//console.log('oculta aside');
	//$('aside#block-region-side-post').css('visibility', 'hidden');
}

// var eventoInterface = setInterval(atualizarMenu, 300);
...
/* $('li.type_system.depth_2.contains_branch').attr("aria-expanded","false"); *
*/
...
//	meus_cursos.className = "tree_item_hovered branch"
...
/* Exemplo mudanca de classe: inicio.className = "tree_item hasicon" */


/*
Incluir num event
if ($('#demo3', iframe.contents()).attr('class') == "collapse in") { 
	$('#bodypage').css('height',    ($('#bodypage').height() + 300) + 'px');
	$('.row-fluid').css('height', + 200px...
} 
else { 
	Adicionar contador
	$('#bodypage').css('height',  '900px')
